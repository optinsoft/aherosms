...
class AsyncHeroSmsException(Exception):
    pass

class NoSMSException(AsyncHeroSmsException):
    pass

class EarlyCancelException(AsyncHeroSmsException):
    pass

class NoNumbersException(AsyncHeroSmsException):
    pass

class WrongMaxPriceException(AsyncHeroSmsException):
    pass

class BannedException(AsyncHeroSmsException):
    pass

class ChannelsLimitException(AsyncHeroSmsException):
    pass

class CanceledException(AsyncHeroSmsException):
    pass

class AsyncHeroSms:
    def __init__(self, apiKey: str, apiUrl: str = 'https://hero-sms.com/stubs/handler_api.php', logger: logging.Logger = None, http_timeout: int = 15,
        ...
        self.iso_country_dict = {
            ...
            "NI":"90",  "NL":"48",  "NO":"174", "NP":"81",  "NU":"204", "NZ":"67",  "OM":"107", "PA":"112", "PE":"65", 
            "PG":"79",  "PH":"4",   "PK":"66",  "PL":"15",  "PR":"97",  "PT":"117", "PY":"87",  "QA":"111", 
            "RE":"146", "RO":"32",  "RS":"29",  "RU":"222", "RW":"140", "SA":"53",  "SC":"184", "SD":"98", 
            ...
            "XK":"203", "YE":"30",  "ZA":"31",  "ZM":"147", "ZW":"96"
        }
            ...
            if len(respList) > 0:            
                ...
                if successCode.endswith('_'):
                    if not code.startswith(successCode):
                        AsyncHeroSms.raiseHeroSmsException(code, respList, noSmsCode)
                else:
                    if code != successCode:
                        AsyncHeroSms.raiseHeroSmsException(code, respList, noSmsCode)
            else:
            ...
            while retry:
                ...
                try:
                    ...
                    if retry_num <= self.connection_error_retries:
                        retry = True
                        if not self.logger is None:
                            self.logger.warning(f"[AsyncHeroSms] {type(e).__name__}: {str(e)}")
                    else:
                        raise
        return wrapper
        ...
        async with aiohttp.ClientSession(connector=conn, raise_for_status=False, timeout=aiohttp.ClientTimeout(total=self.http_timeout)) as session:
            async with session.get(url, timeout=self.http_timeout, proxy=self.http_proxy) as resp:
                if resp.status != 200:
                    ...
                    raise AsyncHeroSmsException(f"Request failed:\nStatus Code: {resp.status}\nText: {respText}")
                try:
                    respText = await resp.text()
                    self.logRequest(query, {'status':resp.status,'text':respText})
                ...
                return AsyncHeroSms.checkResponse(respList, successCode, noSmsCode)
    ...
    async def getCountries(self):
        query = {'action':'getCountries','api_key':self.apiKey}
        respJson = await self.doJsonRequest(query)
        return {"response": 1, "countries": respJson}

    async def getServicesList(self):
        query = {'action':'getServicesList','api_key':self.apiKey}
        respJson = await self.doJsonRequest(query)
        return {"response": 1, "countries": respJson.get('services')}
